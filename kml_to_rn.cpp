 ...
 struct RoadNetwork kml_to_rn(std::string kml_file, double node_spacing)
{
  ...
  while ( std::getline(in,line) )
  {
   ...
   //****************************************************************************************
   //Merge Roads: KML is manual, may contain a combination of road segments and full roads
   //****************************************************************************************
   
  /* std::vector< std::vector < std::tuple<double,double,double> > > road_map_merge;
   std::vector < std::tuple<double,double,double> > road_merge;
   for(int i = 0; i < rn.road_map.size(); i++)
  {
    rn.road = rn.road_map[i];
    rn.point2 = rn.road[1];
    for(int k = i+1; k < rn.road_map.size(); k++)
    {
      std::vector < std::tuple<double,double,double> > road_merge = rn.road_map[k];
      std::tuple<double,double,double> r_beg = road_merge[0];
      std::tuple<double,double,double> r_end = road_merge[road_merge.size()-1];
     
	  //Compare points
	  if(r_beg == rn.point2)
	  {
	    //Merge roads
	    road_merge.insert(road_merge.begin(), rn.road);
	    rn.road_map.erase(rn.road_map.begin(), rn.road_map.begin()+k);
	  }
	  if(r_end == rn.point2)
	  {
	    road_merge.insert(road_merge.end(), rn.road);
	    rn.road_map.erase(rn.road_map.begin(), rn.road_map.begin()+k);
	  }
   ...
   //Generate a list of nodes
    ...
    //Fill in graph with proper number of nodes
    rn.GRAPH = Graph(rn.coordtoNode.size());
   ...
   std::cout << "vertex_index to vertex_label: " << "\n\n";
    for (rn.vp = vertices(rn.GRAPH); rn.vp.first != rn.vp.second; ++rn.vp.first)
    {
      ...
      //std::cout << std::get<0>(rn.point1) << ", " << std::get<1>(rn.point1) << "," << std::get<2>(rn.point1) << "\n";
      //std::cout << "V: " << rn.index[rn.v] << " Node ID: " << std::to_string(rn.coordtoNode.at(rn.point1)) << "\n";
      
      //Extract node tuple from nodetoCoord by index (or ID)
      rn.point1 = rn.nodetoCoord.at(t);
      ...
      std::cout << get(vertex_index, rn.GRAPH, rn.v) << " ---> " << get(vertex_label, rn.GRAPH, rn.v) << "\n";
  ...
  //Create EdgePAIRS, compute Edge distance, and compute Edge slope
    ...
    std::cout << "Edges to Edge Source: " << "\n\n";
    //Add edges to graph
    for (int i = 0; i < num_edges; ++i)
    {
      Edge e = add_edge(rn.edgePAIR_vec[i].first, rn.edgePAIR_vec[i].second, rn.GRAPH).first;
       
      std::cout << rn.index[source(e, rn.GRAPH)] << "--->" << rn.index[target(e, rn.GRAPH)] << 
      " ===== " << rn.edgePAIR_vec[i].first << "--->" << rn.edgePAIR_vec[i].second << "\n";
    }
 
    //Associate each edge with its corresponding length and slope
    int i = 0;
     std::cout << "Edges to slope: " << "\n\n";
